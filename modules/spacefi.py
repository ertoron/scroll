...
from config import SPACEFI_ROUTER_ABI, SPACEFI_CONTRACTS, SCROLL_TOKENS
...
class SpaceFi(Account):
    ...
    async def swap_to_token(self, from_token: str, to_token: str, amount: int, slippage: int):
        ...
        contract_txn = await self.swap_contract.functions.swapExactETHForTokens(
            min_amount_out,
            [self.w3.to_checksum_address(SCROLL_TOKENS[from_token]),
             self.w3.to_checksum_address(SCROLL_TOKENS[to_token])],
            self.address,
            deadline
    ...
    async def swap_to_eth(self, from_token: str, to_token: str, amount: int, slippage: int):
        token_address = self.w3.to_checksum_address(SCROLL_TOKENS[from_token])
    ...
    async def add_liquidity(
        ...
        await self.approve(2 ** 128, SCROLL_TOKENS["USDC"], SPACEFI_CONTRACTS["router"])

        tx_data = await self.get_tx_data(amount_wei)

        transaction = await self.swap_contract.functions.addLiquidityETH(
            self.w3.to_checksum_address(SCROLL_TOKENS["USDC"]),
            amount_wei,
            0,
            0,
            self.address,
